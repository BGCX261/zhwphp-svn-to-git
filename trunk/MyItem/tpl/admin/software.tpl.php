...
.news-list .th2{ width:10%;}
.news-list .th3{ width:30%;}
/*.news-list .th4{ width:20%;}*/
.news-list .th5{ width:45%;}
...
<table border="0" cellspacing="1" cellpadding="0" class="news-list" ondblclick="$('#addform').toggle();">
	<tr>
		...
		<th class="th2" nowrap>所属分类</th>
		<th class="th3" nowrap>软件名称</th>
	...
	<form method="POST" action="<?php echo URL; ?>">
	    ...
	    <td><select name="tid">
	        <option value="1" <?php if ($tid == 1) echo 'selected'; ?>>前台</option>
	        <option value="2" <?php if ($tid == 2) echo 'selected'; ?>>后台</option>
	    </select></td>
...
<!--数据循环开始 -->
	...
	<tr>
		...
		<td class="td2" nowrap><?php echo $type[$tid]; ?></td>
		<td class="td3" nowrap><?php echo $title; ?></td>
		<td class="td5" nowrap><?php echo String::substr($url,0,50); ?></td>
